    ...
    <>
      <header className={`flex z-50 fixed justify-between top-0 w-full items-center lg:px-16 px-4 py-4 ${isScrolled ? 'bg-white' : 'bg-transparent'}`}>
        ...
        <Link className='sm:hidden' href={'/auth'}>
          <button className='bg-purple flex text-white font-medium p-3 px-8 rounded-3xl ml-5'>Start Learning<img className='ml-3' src="/east.png" alt="" />
          </button>
        </Link>
      ...
      {open &&
        <div className='fixed z-50 top-24 left-0 right-0 p-6 bg-purple text-white w-[90%] mx-auto rounded-md'>
          ...
          <button className='bg-white text-purple font-medium p-2 px-8 rounded-md w-full'>Start Learning</button>
        </div>
      }
    </>
  );
